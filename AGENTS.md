# Quai

PaaS mono-conteneur déployable dans Coolify : pousser un dossier statique, une
fonction ou un service persistant en une commande, chaque projet isolé dans son
compte UNIX, son namespace réseau, son cgroup et son quota disque.

## Agent skills

### Issue tracker

Les issues et specs vivent dans les GitHub Issues de `atinseau/quai`, via la CLI `gh`.
See `docs/agents/issue-tracker.md`.

### Triage labels

Les cinq rôles canoniques, chaque label portant le nom de son rôle.
See `docs/agents/triage-labels.md`.

### Domain docs

Single-context : `CONTEXT.md` à la racine et `docs/adr/`.
See `docs/agents/domain.md`.
